...
// PATCH /api/admin/leads/[id] { status?, assignedToId?, note? }
// Returns { lead, brokerAlert? } — brokerAlert present when a WARM transition
// triggered a broker notification.
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  const session = await getSession()
  let body: { status?: string; assignedToId?: string | null; note?: string }
  try {
    body = await req.json()
  } catch {
  ...
  try {
    ...
    return NextResponse.json({ lead: updated, brokerAlert })
  } catch (err) {
